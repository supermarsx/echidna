...
- For build, control-plane topology, and signing facts, treat `docs/developer_readme.md` and
  `docs/signing.md` as the source of truth (the control service is hosted in-process inside the
  companion APK; native libraries are cross-compiled per ABI via `tools/build_native_ndk.sh`).
