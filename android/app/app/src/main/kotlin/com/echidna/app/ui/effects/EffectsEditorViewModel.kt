...
class EffectsEditorViewModel : ViewModel() {
    ...
    /**
     * Adds an effect stage to the active preset's chain. Each stage id is unique (at most one of
     * each), so this is a no-op if the stage is already present. The chain is kept sorted into the
     * native engine's fixed processing order (see [CANONICAL_ORDER]) so the persisted preset always
     * mirrors the DSP pipeline the audio actually flows through.
     */
    fun addModule(moduleId: String) {
        val preset = activePreset.value
        if (preset.modules.any { it.id == moduleId }) return
        val module = defaultModule(moduleId) ?: return
        val modules = (preset.modules + module).sortedBy { canonicalIndex(it.id) }
        repo.updatePreset(preset.copy(modules = modules))
    }

    /** Removes an effect stage from the active preset's chain entirely. */
    fun removeModule(moduleId: String) {
        val preset = activePreset.value
        if (preset.modules.none { it.id == moduleId }) return
        repo.updatePreset(preset.copy(modules = preset.modules.filterNot { it.id == moduleId }))
    }

    private fun canonicalIndex(id: String): Int =
        CANONICAL_ORDER.indexOf(id).let { if (it < 0) CANONICAL_ORDER.size else it }

    /** Sensible enabled-by-default parameters for a freshly added stage. */
    private fun defaultModule(moduleId: String): EffectModule? = when (moduleId) {
        "gate" -> EffectModule.Gate(true, -50f, 5f, 120f, 3f)
        "eq" -> EffectModule.Equalizer(true, defaultEqBands(), 5)
        "comp" -> EffectModule.Compressor(true, EffectModule.CompressorMode.MANUAL, -26f, 3f, 6f, 8f, 160f, 3f)
        "pitch" -> EffectModule.Pitch(true, 0f, 0f, EffectModule.PitchQuality.LL, preserveFormants = true)
        "formant" -> EffectModule.Formant(true, 0f, intelligibilityAssist = true)
        "autotune" -> EffectModule.AutoTune(true, MusicalKey.C, MusicalScale.MAJOR, 20f, 20f, 0f, true, 80f)
        "reverb" -> EffectModule.Reverb(true, 12f, 20f, 5f, 12f)
        "mix" -> EffectModule.Mix(true, 60f, 0f)
        else -> null
    }

    private fun defaultEqBands(): List<Band> = listOf(
        Band(120f, 0f, 1f),
        Band(500f, 0f, 1f),
        Band(1000f, 0f, 1f),
        Band(3500f, 0f, 1f),
    ...
    companion object {
        /** Fixed native DSP pipeline order (see native/dsp/src/engine.cpp): audio flows through
         *  the enabled stages in exactly this sequence. Used to sort the chain for add/display. */
        val CANONICAL_ORDER = listOf(
            "gate", "eq", "comp", "pitch", "formant", "autotune", "reverb", "mix"
        )
    }
}
