...
- If release signing secrets are absent, the APK build keeps the documented debug-signing fallback
  and produces a non-distributable release APK.
...
- Build the release-signed APK (`assembleRelease` with a real keystore — see
