...
namespace
{
    ...
    bool LoadDspLocked(DspBridge &dsp)
    {
        ...
        const char *candidates[] = {
            ...
            "/system/lib64/libech_dsp.so",
            "/data/adb/modules/echidna/system/lib64/libech_dsp.so",
            "/data/adb/modules/echidna/libs/arm64-v8a/libech_dsp.so",
#elif defined(__arm__)
            "/system/lib/libech_dsp.so",
            "/data/adb/modules/echidna/system/lib/libech_dsp.so",
            "/data/adb/modules/echidna/libs/armeabi-v7a/libech_dsp.so",
#elif defined(__x86_64__)
            "/system/lib64/libech_dsp.so",
            "/data/adb/modules/echidna/system/lib64/libech_dsp.so",
            "/data/adb/modules/echidna/libs/x86_64/libech_dsp.so",
#endif
            "/data/local/tmp/echidna-dsp.so",
        };
        const char *last_error = nullptr;
        for (const char *candidate : candidates)
        {
            dlerror();
            dsp.handle = dlopen(candidate, RTLD_NOW | RTLD_LOCAL);
            if (dsp.handle)
            {
                break;
            }
