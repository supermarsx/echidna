    ...
    namespace hooks
    {
        ...
        namespace
        {
                                  ...
                                  uint32_t frames)
            {
                ...
                {
                    return RouteFloatCaptureBufferInPlace(buffer,
                                                          frames,
                                                          cfg.sample_rate,
                                                          channels,
                                 ...
                                 int64_t timeout_ns)
            {
                ...
                if (!processed)
                {
                    result = gOriginalWrite ? gOriginalWrite(stream, buffer, frames, timeout_ns)
                                            : 0;
                }
                RecordHookCall(state,
                               timing,
            ...
            int ForwardCallback(void *stream, void *user, void *audio_data, int32_t num_frames)
            {
                ...
                // Capture context before calling the app's callback so the
                // processed buffer is delivered to the app.
                const StreamConfig cfg = CachedConfig(stream);
                if (audio_data && num_frames > 0 && ShouldProcess(cfg))
                {
                    processed = ProcessPcmBuffer(cfg,
                                                 audio_data,
