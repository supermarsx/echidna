...
namespace echidna
{
    namespace utils
    {
         ...
         *
         * Defined at echidna::utils scope (not inside the anonymous namespace) so it
         * legally completes the SharedLayout member forward-declared in the header;
         * a member type cannot be defined inside an unrelated (anonymous) namespace.
         */
        ...
        TelemetrySharedMemory::~TelemetrySharedMemory()
        {
            if (layout_)
            {
                ReleaseSharedRegion(kTelemetrySharedMemoryName);
                layout_ = nullptr;
            }
            fd_ = -1;
            writable_ = false;
        ...
        void TelemetrySharedMemory::ensureInitialized()
        {
            ...
            // bionic has no shm_open; back the telemetry region with an anonymous
            // memfd kept in a process-global name registry (see android_shared_memory.h).
            AndroidSharedRegion region = AcquireSharedRegion(kTelemetrySharedMemoryName, layout_size_);
            if (!region.addr)
            {
                return;
            }
